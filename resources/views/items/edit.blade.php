...
<div class="card">
    <div class="card-header bg-white">
        <h5 class="card-title mb-0">Edit Barang: {{ $item->name }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('items.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <div class="border border-2 rounded text-center p-4" style="border-style: dashed !important; background-color: #fcfcfc; cursor: pointer;" onclick="document.getElementById('image').click()">
                    ...
                    <p class="mb-1 text-muted">Klik untuk memilih foto baru, atau seret file ke sini</p>
                    <small class="text-muted d-block mb-3">Format: JPG, PNG — Maks. 2 MB. Kosongkan jika tidak ingin mengubah foto.</small>
                    <button type="button" class="btn btn-outline-secondary btn-sm bg-white" onclick="event.stopPropagation(); document.getElementById('image').click()">Pilih Foto Baru</button>
                ...
                @if($item->image)
                    <div class="mt-3 text-center">
                        ...
                        <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name }}" class="img-thumbnail" style="max-height: 120px; border-radius: 8px;">
                    </div>
                @endif
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control @error('current_stock') is-invalid @enderror" id="current_stock" name="current_stock" value="{{ old('current_stock', $item->current_stock) }}" min="0" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control @error('min_stock') is-invalid @enderror" id="min_stock" name="min_stock" value="{{ old('min_stock', $item->min_stock) }}" min="0">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control @error('sell_price') is-invalid @enderror" id="sell_price" name="sell_price" value="{{ old('sell_price', $item->sell_price) }}" min="0">
                    @error('sell_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="buy_price" class="form-label">Harga beli (Rp)</label>
                    <input type="number" class="form-control @error('buy_price') is-invalid @enderror" id="buy_price" name="buy_price" value="{{ old('buy_price', $item->buy_price) }}" min="0">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('weight_size') is-invalid @enderror" id="weight_size" name="weight_size" value="{{ old('weight_size', $item->weight_size) }}">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('storage_location') is-invalid @enderror" id="storage_location" name="storage_location" value="{{ old('storage_location', $item->storage_location) }}">
